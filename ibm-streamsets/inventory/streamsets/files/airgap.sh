...
find_images_to_mirror() {
    ...
    if [[ ${images_count} == 0 ]]; then
      ...
      cleanup_mirrored_csv_files_from_current_run
      echo "[ERROR] Found 0 images to be mirrored. Make sure that the download directory path is correct and that any specified groups are valid."
      exit 11
    fi
}
...
check_filter() {
    if [[ $FILTER_FAILS == true ]]; then
        ...
        # On error we need to clean up our mirrored csv file
        cleanup_mirrored_csv_files_from_current_run
        echo "[ERROR] Did not find any images matching the specified filter: [--filter $filter_list]. Correct the filter or remove the filter flag to mirror all images."
        exit 11
    fi
}
...
process_case_csv_file() {
    ...
    if [[ ! -z ${IMAGE_MIRROR_ACTION_CALLED} ]] && [[ $(wc -l < "${tmp_csv_file}") -gt 1 ]]; then
