...
validate_file_exists() {
    local file=$1
    [[ ! -f ${file} && ! -L ${file} ]] && { err_exit "${file} is missing, exiting deployment."; }
}
...
get_case_name() {
    ...
    caseName=$(echo $caseName | awk -F':' '{print $2}')     #get everything after ':', ex. `name: ibm-cp4d-ccs` becomes `ibm-cp4d-ccs`
