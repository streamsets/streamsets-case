...
OPTIONS:
 ...
 ARGS per Action:
    ...
    --dryRun                 : print the actions that would be taken and exit without writing to the destinations
      --dryRunOutput           : if specified, the output of dry run will be saved to given directory
      ...
      --log-level              : set the log level, options are ERROR, WARN, INFO, DEBUG (default is INFO)
...
configure_creds_airgap() {
    ...
    "${scriptDir}"/airgap.sh registry secret -c -u "${user}" -p "${pass}" "${registry}" "${dryRun}" \
...
image_info() {
    ...
    "${scriptDir}"/airgap.sh image mirror --show-registries-namespaces --dir "${inputcasedir}" "${dryRun}" \
        ${dryRunOutput:+'--dry-run-output' "$dryRunOutput"} \
        ${log_level:+'--log-level' "$log_level"}
}
